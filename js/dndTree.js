var dndTree = (function() {
    ...
    function setChildrenAndUpdateForTrack(node) {
        var tracks;
        AE.getRecommendationForTracks(node.track.id).then(function(tracks) {
            if (!node.children) {
                node.children = []
            }
    ...
    function initWithData(from, to) {
        ...
        if(from.track) {
          to.track = from.track;
          exploredArtistIds.push(to.track.id);
        }
        if (from.genre) {
            to.genre = from.genre;
        }
    ...
    return {
        ...
        "setRootTrack" : function(track) {
          exploredArtistIds = []
          root = initWithTrack(track);
          root.x0 = viewerHeight / 2;
          root.y0 = 0;
          update(root);
          centerNode(root);
